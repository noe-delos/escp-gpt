...
const SuggestionButton: React.FC<SuggestionButtonProps> = ({ text, icon, color, onClick }) => (
  <motion.button
    ...
    whileTap={{ scale: 0.97 }}
    className="px-4 py-2 rounded-full bg-[#779aff]/20 text-[#224fd1] hover:bg-[#779aff]/10 text-sm flex items-center gap-2"
    onClick={onClick}
  >
...
const SendButton: React.FC<SendButtonProps> = ({ disabled, type = "submit" }) => (
  <motion.button
    ...
    type={type}
    disabled={disabled}
    className="rounded-full bg-[#303cac] hover:bg-[#37419c] h-10 w-10 flex items-center justify-center text-white disabled:cursor-not-allowed"
  >
              ...
              <div className="py-20 text-center max-w-4xl mx-auto">
                ...
                >
                  <form ref={formRef} onSubmit={customSubmit} className="relative">
                    ...
                    <div className="absolute left-3 bottom-4 bg-[#779aff]/20 text-[#224fd1] rounded-full px-3 py-1 flex items-center gap-2 text-sm pointer-events-none">
